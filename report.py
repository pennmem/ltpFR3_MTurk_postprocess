...
def plot_elis(s, err=None):
    plt.subplot(7, 3, 15)
    if err is None:
        if not np.isnan(s['sv12']) and not np.isnan(s['fv12']):
            plt.plot([1, 2], [s['sv12'], s['fv12']], 'ko-', label='Visual')
        if not np.isnan(s['sa12']) and not np.isnan(s['fa12']):
            plt.plot([1.02, 2.02], [s['sa12'], s['fa12']], 'ko--', markerfacecolor='white', label='Auditory')
        if not np.isnan(s['sv24']) and not np.isnan(s['fv24']):
            plt.plot([3, 4], [s['sv24'], s['fv24']], 'ko-')
        if not np.isnan(s['sa24']) and not np.isnan(s['fa24']):
