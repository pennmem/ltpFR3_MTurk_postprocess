...
def pli_recency(intrusions, subjects, nmax, rec_words):
    ...
    :param rec_words: A lists x words matrix, where item (i, j) is the jth word recalled on the ith list
    ...
    for i, s in enumerate(usub):
        ...
        subj_recw = rec_words[subjects == s, :]
        for j in range(len(subj_intru)):
            ...
            encountered = []
            for k in range(ll):
                if subj_intru[j][k] > 0 and subj_recw[j][k] not in encountered:
                    encountered.append(subj_recw[j][k])
                    if subj_intru[j][k] <= nmax:
